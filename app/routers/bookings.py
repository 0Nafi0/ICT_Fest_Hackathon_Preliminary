...
def _compute_refund_amount(price_cents: int, refund_percent: int) -> int:
    return (price_cents * refund_percent + 50) // 100
...
def create_booking(
    ...
    with _booking_lock:
        ...
        while db.query(Booking.id).filter(Booking.reference_code == reference_code).first() is not None:
            reference_code = reference.next_reference_code()
...
def cancel_booking(
    ...
    with _booking_lock:
        ...
        refund_amount_cents = _compute_refund_amount(booking.price_cents, refund_percent)
